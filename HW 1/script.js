...
let userWith2000 = users.filter(user => parseFloat(user.balance.replace(/[$,]/g, '')) > 2000);
let phoneNumbers = userWith2000.map(user => user.phone);

console.log("Телефонні номери користувачів з балансом > 2000$:", phoneNumbers);



//Знайти суму всіх балансів користувачів
let totalBalance = users.reduce((sum, user) => sum + parseFloat(user.balance.replace(/[$,]/g, '')), 0);
